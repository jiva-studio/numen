...
The word counts, the two overlaps and the legibility thresholds are constants in `internal/core/cutting`. The character limit is the one value settings reach, and it is derived from the model's input limit.
