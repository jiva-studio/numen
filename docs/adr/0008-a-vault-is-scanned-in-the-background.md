...
**A write transaction takes its lock at BEGIN.** Two processes hold a writer each (ADR-0030), and a transaction that reads before it writes is a snapshot upgraded under lock, which SQLite refuses with `SQLITE_BUSY` without calling the busy handler. So the write pool begins every transaction immediately: a writer whose turn has not come waits out the busy timeout and then fails, and a write is never lost quietly.
