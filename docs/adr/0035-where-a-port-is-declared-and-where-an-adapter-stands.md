...
The core declares its interfaces in one folder, and whether a given one belongs there has been settled a case at a time: by how many callers it has, by which way the call goes through it, by which folder read better on the day. Each answer stood on its own and no two were the same rule, so the next one is argued from the start again.

Two questions are the shape of it. A one-method interface a single use case asks for — is it a port, or is one caller too few? And an adapter that serves the schema while sitting where nothing outside the module can compose it — is it in the wrong folder, or is the folder about something else?

## Decision

### A port is a conversation, and a conversation is not counted in callers

A port is a purposeful conversation between the core and something outside it. *Tell me when the folder changed* is such a conversation; that one use case is the only one who wants telling is a fact about this application, not about the conversation. An interface asked for once and an interface asked for six times are the same kind of thing.
...
Nearly every interface the core declares is a port. The ones that are not are handles a port hands back, each standing for one thing already in flight, and a handle is part of the signature of the conversation that opens it; nothing binds one.
...
The composition root uses values of every port it binds: it builds the adapter and hands it over. It is therefore a consumer of all of them, and the folder holding what that one consumer must see is the port folder. This is the whole of the rule, and it is the rule [A hexagonal core in Go](0004-a-hexagonal-core-in-go.md) already states — an interface an adapter is bound to at the composition root is declared as a port, and a one-method interface a single use case needs, that nothing binds, is declared beside that use case.

The four whose only callers are adapters are decided by the same sentence. The window and the folder dialog are asked for by the window's adapter and the tool endpoint's; the agent by the window's and the review window's; the progress of indexing by the window's and the composition root. Two adapters must see one interface, and an adapter takes no other adapter, so the only place both can see it is the core. They are ports because more than one adapter has to see them, not because of which side of the hexagon they sit on.

### A port is named in the core's own language, whatever that costs the caller

A conversation is held in the words of what it is about. The port that pulls text out of a document speaks of a highlight's box and a part's start, the one that reads a scan of a block of a page, the one that puts a reading right of a batch — and each of those words belongs to a package of the core. A caller that wants one port is compiled from all of them, and that is the price.
...
A held adapter is one nothing outside this module composes. Where it stands is Go's visibility and nothing else, and it says who may build the adapter — never which way a call goes through it.
...
The theme adapter is a driving adapter: it answers the schema's theme calls, and it watches a folder to answer one of them. Nothing outside the core composes it — the two windows mount it, and an application reaches it through the composition root — so it is held inside the module, and it would be held there if it were driven, and it would be public if an application built one for itself.
...
- **Every port is asked for somewhere else.** A port left declared after its last caller went is indirection standing on its own, and the composition root goes on binding an adapter to it.
...
- **A driving adapter can be held inside the module.** Reading the tree does not say which way an adapter faces, and the layer guard is where that is written down.
...
**Deciding by the layer of the caller: an interface only an adapter asks for is not a port.** Rejected: two adapters would then share it by one naming the other, which is the thing the layer rule exists to refuse, and the agent this application starts is a port.

**Declaring the words a port is named in at the boundary, so the port folder is built from nothing.** Rejected: a highlight's box and a block of a page are the domain's, and a second set of them is two vocabularies to keep in step for a shorter import list.

**Making the theme adapter public, on the ground that it is driving.** Rejected: nothing outside the core composes it, so making it public puts an adapter on the core's surface that no application names.
