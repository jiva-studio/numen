...
| url | A file holding a web address and nothing else, under `.url` — [Importing an address](importing.md), [A url is a source of its own](adr/0038-a-url-is-a-source-of-its-own.md). It is a source beside a book and a recording: its text is what was fetched from that address, and it holds no body to write in. | link, link note |
| address | Where a url points: an `http` or `https` URL in the one form every spelling of it reaches. It is what everything fetched for that file is named by, and its fingerprint is the `hash` of the source. | |
