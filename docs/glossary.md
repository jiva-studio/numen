...
| producer | What made a source's text, where the source's own bytes are not it: `asr` for a recording heard, the recogniser's name for a scan read. It is the one word everywhere — the `producer` column in the storage, `port.SourceText.Producer` in the core — it is half of the name a reading's files are kept under, and it is empty for the ordinary source that is its own text. | reader, extractor, text_from |
