...
| layout (of a book) | Whether a book's documents reflow or are pages laid out once and drawn as they stand. The two values are the format's own: `reflowable` and `pre-paginated`. | fixed |
| page progression | Which way a book's pages advance, `ltr` or `rtl`, and unsaid for a book that names neither. It is the spine's own word. | direction |
| linear | Whether a document of the spine stands in the reading order. One that does not is a note or an appendix; its text is in the book's text all the same, and only a reader turning pages passes it by. | |
