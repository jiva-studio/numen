...
| Term | What it is | Never called |
| --- | --- | --- |
| artifact | Data that cannot be reproduced locally, deterministically and for free. Lives in the vault. It is also the name one such file is kept under: the producer, then the hash of what was read. | |
...
| index | The cache. | |
| source | A thing the index holds text for. A note, a book and a recording are the three kinds. | |
| producer | What made a source's text, where the source's own bytes are not it: `asr` for a recording heard, the recogniser's name for a scan read. It is the one word everywhere — the `producer` column in the storage, and `Producer` on `domain.Source`, `domain.Passage` and `port.SourceText` in the core — it is half of the name a reading's files are kept under, and it is empty for the ordinary source that is its own text. What a recipe names is the reader, which is the format's own extraction and a different thing under a word that would be taken for this one. | reader, extractor, text_from |
| reading | What a model read off the pages of a scanned document, and the files it is kept in — [Reading](reading.md). It is the artifact a chunk of that document is a place in, named by the producer and the hash of the bytes read, and it is what a proofreader puts right. A reading is to a scanned document what a transcript is to a recording. | recognition |
| recognition | One run of a model over a scanned document's pages, asked for by a person and never begun by the application. It is also the settings section `indexing.recognition` and the adapter that reads it, in a row with `transcription` and `proofreading`, which are named the same way. What a recognition produces is a reading: the run and the artifact take different words on purpose, as `proofread` and `proofreading` do. | |
...
| link | A note carrying `type: link` and an address in `url` — [Importing an address](importing.md), [A link is a note that carries an address](adr/0038-a-link-is-a-note-that-carries-an-address.md). Its body is the person's prose, and it is searched together with what was fetched from that address. | link note |
| address | Where a link note points: an `http` or `https` URL in the one form every spelling of it reaches. It is what everything fetched for that note is named by, and it is the `address` column of a note's row. | url |
| captions | The words published with a video, kept as WebVTT — [Importing an address](importing.md). They are the artifact a chunk of that link note is a place in, and they are what a machine here heard only when nobody published any. | |
| transcript | What a model heard in a recording, written down as WebVTT — [Transcribing](transcribing.md), [A transcript is WebVTT](adr/0031-a-transcript-is-webvtt.md). It is the artifact a chunk of that recording is a place in, named by the hash of the recording's bytes, and it is what a proofreader puts right. A transcript is to a recording what a reading is to a scanned document. | |
...
| proofread | Asking a second model to put right the text a first one produced, and writing its corrections beside the artifact — [Proofreading](proofreading.md). It is the word for the run wherever it is named: the `proofread` facet of an asset, the command, and **Proofread transcript** on screen. What it does to one line is put that line right. Writing a transcript back as a person edited it in the window is not a proofreading. | |
| proofreading | The settings section `indexing.proofreading` and what parses it, which is the other half of `proofread`: the domain of a correction on one side, the settings and the HTTP on the other. The noun and the verb are asymmetric on purpose — the adapter is named for the section it reads, in a row with `recognition` and `transcription`, which are named the same way. | |
...
| drop (a transcript) | Taking away everything listening to a recording produced: what a model heard, what a person put right, the record of what listened, the answer and the chunks cut from any of them — [Transcribing](transcribing.md). The word is for the code: on screen it is **Delete transcript**. | |
