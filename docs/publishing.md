...
Where the page and the builds are served from, and what has to exist for the
workflows to put them there.
...
exactly to the extent that a pull zone points at it, and the page and the
builds have a store each — nothing in one can be reached through the other.
...
Both stores are in Falkenstein, so the endpoint is `storage.bunnycdn.com`. Both
zones serve from every continent and hold what they were given for thirty days;
the builds' zone is on the volume tier, which is what large files that are
rarely fetched should be on.

The page goes into the root of its store. The builds go into `latest/`.
...
Secrets: `BUNNY_STORAGE_KEY` and `BUNNY_SITE_STORAGE_KEY` are the two stores'
own passwords; `BUNNY_API_KEY` is the account key, which is what purging asks
for. They are in 1Password beside the account key they came from.
...
Each pull zone answers on its own `b-cdn.net` address today. To answer on
`numen.md` and `dl.numen.md`, add the hostname to the pull zone, point a CNAME
at the zone's address, and turn on the certificate.
