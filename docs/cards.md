...
`learned` is what the preset counts as a card learned, and the value it reads stands under the key it names. Under `interval` a card is learned once the interval it is sent away for reaches `interval` days; under `retention` it is learned once the chance of recalling it today is at or above `retention`. The rule not named keeps its value and takes no part until it is chosen again, and a preset naming no rule at all counts by `interval` at 21 days. How many cards stand learned today is answered by the rule in force. How long until all of them are is answered only under `interval`, where a card passes the threshold once and stays past it; under `retention` a card drops under the target as it fades and rises over it when it is answered, so there is no day the whole deck stands learned at once and none is named. It is not named against a `retention` goal either: moving that control moves the scheduler, and the day a card passes an interval is the reviews it takes times the space between them, so it steps wherever another review is wanted. The share of the deck learned over the run stands there instead. Every value of the day is right where it is asked, and the picture is drawn only where it is steady under the control beneath it.
