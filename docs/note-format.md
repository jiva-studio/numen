...
| `type` | Which of five this note is: `note`, `deck`, `stencil`, `preset` or `link`. A note carrying none is a `note`. | [The stencil, the deck and the card](adr/0026-the-stencil-and-the-deck.md), [A preset is a note, and one arithmetic schedules it](adr/0029-the-preset.md) |
| `url` | The address this note points at. Read on a link and nowhere else, in the one form every spelling of it reaches. A link carrying none is a problem against the note. | [A link is a note that carries an address](adr/0038-a-link-is-a-note-that-carries-an-address.md) |
