...
- [ ] **Решение, не уборка:** перенести те одиннадцать под `internal/` — значит сузить публичную поверхность библиотеки, и компилятор станет держать то, что сейчас держит тест. Технически это `git mv` и правка путей внутри ядра; вопрос в том, считаем ли мы ядро библиотекой для кого-то ещё.
...
- [ ] `PlexShowing` вместе с семьёй — объявлен в `libs/ui/src/features/plex/showing.ts`, то есть это раздел 14.
