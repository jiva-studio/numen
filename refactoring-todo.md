...
- [ ] **Блокер, чинится.** `shared/lists.ts`: перенеся `StepRow`/`StepGroup` вниз, я превратил живое нарушение (`entities/settings` строит строки палитры) в невидимое. Это ровно тот обход, от которого предостерегает роль: тип вытолкнут в `shared/` там, где им владел один из двух, а реэкспорт отмывает запрещённый импорт через законный. Лечится разрезом `entities/settings` по четырём швам, как и записано в «Решениях».
...
- [ ] `shared/result.ts` (`Result`, `ok`, `err`) не импортирует никто, кроме собственного теста. Это реализация конвенции из `AGENTS.md` §8, поэтому решение не техническое: либо ответы выше переходят на неё, либо конвенция и файл уходят вместе.
