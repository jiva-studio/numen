...
export function asPlex(neighbourhood: NeighbourhoodResponse): PlexNeighbourhood {
  ...
  for (const related of neighbourhood.related) {
    ...
    const line = related.label ? { label: related.label } : {}
    const head = (end: EdgeArrow) => (related.mutual ? { arrow: end } : {})
    if (seat === 'parent' || seat === 'jump') {
      edges.push({ from: id, to: focus.id, ...line, ...head('from') })
    } else if (seat === 'child') {
      edges.push({ from: focus.id, to: id, ...line, ...head('to') })
    } else {
