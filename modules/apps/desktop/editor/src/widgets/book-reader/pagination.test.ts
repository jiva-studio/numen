...
describe('pagination and document lookup', () => {
  it('calculates page number based on byte offset', () => {
    expect(pageAt(1000, 10, 0)).toBe(1)
    expect(pageAt(1000, 10, 999)).toBe(1)
    expect(pageAt(1000, 10, 1000)).toBe(2)
    expect(pageAt(1000, 10, 50000)).toBe(10)
    expect(pageAt(0, 10, 500)).toBe(0)
    expect(pageAt(1000, 0, 500)).toBe(0)
  })
  ...
  it('finds spine document containing offset', () => {
    ...
    expect(documentAt(docs, 0)?.path).toBe('ch1.xhtml')
    expect(documentAt(docs, 500)?.path).toBe('ch1.xhtml')
    expect(documentAt(docs, 1000)?.path).toBe('ch2.xhtml')
    expect(documentAt(docs, 3000)?.path).toBe('ch3.xhtml')
  })
  ...
  it('builds table of contents entries from parts, printed pages, or spine docs', () => {
    ...
    expect(contentsOf(bookWithParts, words)).toEqual([
    ...
    expect(contentsOf(bookWithPrinted, words)).toEqual([
