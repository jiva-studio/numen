...
describe('a passage of a book reached', () => {
  it('opens the book and sends the tab to the stretches asked about', async () => {
    const reach = vi.fn()
    ...
    const stretches: readonly Stretch[] = [{ start: 3_600, length: 42 }]
    opens()?.('library/Mahabharata.epub', stretches)
    await settles()

    expect(opened).toStrictEqual([`${BOOK} library/Mahabharata.epub`])
    expect(reach).toHaveBeenCalledWith({ start: 3_600, length: 42 })
  })
})
