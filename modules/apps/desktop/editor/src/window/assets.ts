/**
 * What a file of the vault is, for whatever opens it: a document's pages, the
 * documents a book that reflows is read in, a recording's duration and where
 * its sound is played from, and its transcript.
 *
 * The application serves the bytes over addresses of its own, because the
 * window is drawn from a scheme a browser loads neither pictures nor sound
 * through.
 */
...
import type { Books, SpineDocument } from '../book-tab/open'
import type { Recordings } from '../shared/media/transcript'
...
const served = {
  ...
  /** What a book that reflows is: the documents it is read in and what it names. */
