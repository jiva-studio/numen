...
// `Recipe` and `TextPath` are empty until something computes them.
type Source struct {
	Path   string
	Kind   string
	Size   int64
	...
	// TextPath names the file this source's chunks are places in, when it is
	// not the source's own bytes. It is a name in the application's store.
	TextPath string
}
...
func (r *Repository) SaveSource(ctx context.Context, vaultID string, s Source) error {
	...
	if err := tx.QueryRowContext(ctx, stmt.Get("save_source"),
		vault, s.Path, s.Kind, s.Size, s.MTime, nullable(s.Hash), nullable(s.Recipe), nullable(s.TextPath)).Scan(&row); err != nil {
...
func (r *Repository) SaveExtraction(ctx context.Context, vaultID string, s Source, windows []Window) error {
	...
	var source int64
	if err := tx.QueryRowContext(ctx, stmt.Get("save_source"),
		vault, s.Path, s.Kind, s.Size, s.MTime, nullable(s.Hash), nullable(s.Recipe), nullable(s.TextPath)).Scan(&source); err != nil {
		return fmt.Errorf("save_source %s: %w", s.Path, err)
	}
