...
type SourceQueries interface {
	...
	// rather than their own bytes, keyed by path and naming that file.
	//
	// A scan asks it to find the ones whose file is gone: the store is a folder
	// on the person's disk and they may empty it.
	Recognised(ctx context.Context, vaultID string, kind domain.SourceKind) (map[string]string, error)
}
