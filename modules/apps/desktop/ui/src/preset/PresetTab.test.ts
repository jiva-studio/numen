...
describe('what a goal of a date draws', () => {
  ...
  const days = (tab: ReturnType<typeof mount>) =>
    (tab.get('.control__backlog').attributes('d') ?? '').split(/[ML]/).length - 1
  ...
  it('keeps one height for the band, whatever place the knob stands on', async () => {
    ...
    const control = tab.get('.control__picture[role="slider"]')
    await control.trigger('keydown', { key: 'End' })
    const tallest = heights(tab.get('.control__backlog').attributes('d') ?? '')
    await control.trigger('keydown', { key: 'Home' })
    const shortest = heights(tab.get('.control__backlog').attributes('d') ?? '')
...
describe('the band of what stands overdue', () => {
  ...
  it('is read and not dragged, so the one control stays the one control', () => {
    ...
    expect(tab.findAll('.control svg')).toHaveLength(2)
  })
...
describe('what the control stands at', () => {
  ...
  it('draws no line and no control while the curve is being worked out', () => {
    ...
    expect(tab.findAll('.control svg')).toHaveLength(0)
...
describe('the settings under the control', () => {
  ...
  it('hands on the rule that was chosen, and is done with it at once', async () => {
    ...
    await tab.get('.preset__choice').trigger('click')
