...
import type { PlexEdge, PlexNeighbourhood, PlexNode, PlexRole } from '@numen/ui'
...
export function asPlex(neighbourhood: Neighbourhood): PlexNeighbourhood {
  ...
  const seated: { id: string; role: PlexRole }[] = []
  for (const related of neighbourhood.related) {
    ...
    seated.push({ id: related.note.path, role })
  }

  const firstParent = seated.find((node) => node.role === 'parent')?.id
  const edges: PlexEdge[] = seated.flatMap(({ id, role }) => {
    if (role === 'parent' || role === 'jump') return [{ from: id, to: focus.id }]
    // A sibling with no parent shown has nothing to hang from.
    if (role === 'sibling') return firstParent ? [{ from: firstParent, to: id }] : []
    return [{ from: focus.id, to: id }]
  })
