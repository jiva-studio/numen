...
/**
 * The flashcards window: the vaults and what each owes, and the cards themselves.
 *
...
import { ahead, called, cards, deckName, rated, said } from './core'
...
const counting_ = async () => {
  counting.value = true
  try {
    const answer = await cards.owing({})
...
const start = async (deck: string) => {
  try {
    const answer = await cards.start({ vaultId: vault.value, deck })
...
const answer = async (how: Said) => {
  ...
  writing.value = true
  try {
    const given = await cards.answer({
      vaultId: vault.value,
      run: run.value,
      card: one.card,
      face: one.face,
...
const takeBack = async () => {
  ...
  writing.value = true
  try {
    await cards.takeBack({ vaultId: vault.value, run: run.value, answer: last })
  } catch (why) {
    failed(why)
    return
  } finally {
...
<template>
  <main class="flashcards">
    ...
    <section v-else-if="over" class="flashcards__over">
      <h1 class="flashcards__over-said">Nothing left today.</h1>
      <p class="flashcards__over-count">{{ done }} answered.</p>
      <Button variant="outline" @click="leave">Back to the decks</Button>
    </section>

    <section v-else-if="card" class="flashcards__session">
      <header class="flashcards__where">
        <span class="flashcards__deck">{{ deckName(card.deck) }}</span>
        <span v-if="card.section">{{ card.section }}</span>
        <span>{{ card.face }}</span>
        <span v-if="!card.seen" class="flashcards__new">new</span>
        <span class="flashcards__left">{{ asked.length - at }} left</span>
      ...
      <div class="flashcards__card" aria-live="polite">
        <Card :front="card.front" :back="card.back" :shown="shown" @show="show" />
      </div>

      <footer class="flashcards__answers">
        ...
        <template v-if="shown">
          ...
          >
            ...
            <span v-if="card.ahead" class="flashcards__ahead" aria-hidden="true">{{
              ahead(card.ahead[how])
            }}</span>
          </Button>
        </template>
        <Button v-else variant="outline" class="flashcards__answer" @click="show">
...
.flashcards__over-said {
  margin: 0;
  font-size: var(--numen-display-size);
  font-weight: 600;
}

.flashcards__over-count {
  margin: 0;
  color: var(--numen-edge-label);
}
