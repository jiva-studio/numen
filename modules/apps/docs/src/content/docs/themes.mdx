...
| `--numen-alarm-bg` | the ground a card carrying one of those stands on. |
...
| `--numen-edge-label` | the words on that line, which follow `--numen-hushed` unless a theme says otherwise. |
