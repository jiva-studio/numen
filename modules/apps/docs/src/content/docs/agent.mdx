...
**It cannot erase a vault from your disk.** Taking a folder away is asked for in front of you. Putting a folder on the list it can do, when you ask: nothing already in the folder is moved or rewritten, and the vault's identity is written into it so the folder is recognised wherever it moves to.
