...
**Settings** in the commands opens a page over the whole file, in the groups the file keeps them
in.

<Shot
  name="settings"
  alt="The numen window: the settings page, in the groups the file keeps them in."
  caption="A row you can change is a control; a row this window does not write says where it stands."
/>

A row here and the command of the same name go through one piece of code, so it makes no
difference which you use. A setting the window does not write yet is drawn where it belongs and
says **Set in numen.json**.

The same settings are in the commands — <kbd>Ctrl</kbd>/<kbd>⌘</kbd>
...
| `indexing.transcription` | how a recording is listened to, and how large one may be to be listened to unasked. See [recordings](/recordings/). |
