...
A reading can be corrected afterwards by a model that speaks the ordinary chat request.
**Nothing does this by default**: name nothing under `indexing.proofreading` and a reading is
used exactly as it was read, with no key and no network.
...
`batch_url` leaves the pages in a queue and collects them later at half the price. A batch
outlives the run that left it, so one left before you closed the application is collected the
next time you open it. Leave it empty and pages are asked for one at a time, and waited for.

`letters_apart` is the safety catch: how far a correction may move a line's letters and still
count as a correction. Over 931 corrections of one book, every correction further apart than
0.30 was damage — text dragged in from the next line, or one corrected word standing in for a
whole line — and everything below it was a genuine fix. A correction further apart than this is
dropped, and that line is left as it was read.
