...
// hearingLine is what one recording's transcription is called, wherever it is
// shown. One recording is one line, and it replaces itself as the words are
// written down.
func hearingLine(path string) string { return "hearing-" + path }

// Listens is what hears a recording, opened when there is one to hear. It is
// told how far the fetching of what it needs has got.
type Listens func(ctx context.Context, tell func(what string, done, total int64)) (port.Transcriber, func() error, error)

// Correcting is what a reading is put right with: what answers, where batches
// are left for it to answer about later, and how much of a reading goes over at
// a time.
//
// Naming no proofreader is a reading used exactly as it was read.
type Correcting struct {
	// Named says whether a profile is named for this kind of reading. A person
	// is offered the run where one is.
	Named bool
	// Automatically says whether a reading is put right without anybody asking.
	Automatically bool
	...
	// Batch is how much of a reading one request carries, Overlap how much of it
	// the request before also carried, and InFlight how many stand out at once.
	Batch, Overlap, InFlight int
	// MaxEditDistance is how far a reply may stand from what was read before it
	// is refused as an answer about something else.
	MaxEditDistance float64
}

// Transcriptions is what one installation listens to recordings with: what
// hears them, what a vault is read and written through, and what puts a
...
type Transcriptions struct {
	...
	// Open hears a recording, and Ready says whether opening it would wait for
	// anything to arrive.
	Open  Listens
	Ready func() bool

	// Unasked is how many bytes a recording may run to and still be listened to
