...
func TestAPickerThePersonClosedAddsNothingAndSaysSo(t *testing.T) {
	...
	if !strings.Contains(out.Says, "picker") {
		t.Errorf("the answer says %q", out.Says)
	}
...
func TestOpeningAVaultMovesTheWindowToIt(t *testing.T) {
	...
	if !out.Opening || !strings.Contains(out.Says, f.second.Name) {
...
func TestOpeningSaysThatTheSessionEnds(t *testing.T) {
	...
	for _, tool := range listed.Tools {
		if tool.Name != "vault_open" {
			continue
		}
		if !strings.Contains(tool.Description, "session ends") {
			t.Errorf("vault_open says %q", tool.Description)
		}
		return
	}
	t.Fatal("vault_open is missing")
}
...
func TestOpeningTheVaultInFrontIsRefused(t *testing.T) {
	...
	why := failing(t, session, "vault_open", map[string]any{"vault": f.first.ID})
