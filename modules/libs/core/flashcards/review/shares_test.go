...
func TestTheSharesOfADayAreHandedOutWhole(t *testing.T) {
	...
	rapid.Check(t, func(t *rapid.T) {
		...
		got := getTotalOwed(review.GetShares(budget, owes))
...
func TestNoDeckTakesMoreOfTheDayThanItOwes(t *testing.T) {
	...
	rapid.Check(t, func(t *rapid.T) {
		budget, owes := drawDivision(t)
		for at, share := range review.GetShares(budget, owes) {
...
func TestADeckOwingMoreNeverTakesLess(t *testing.T) {
	...
	rapid.Check(t, func(t *rapid.T) {
		budget, owes := drawDivision(t)
		out := review.GetShares(budget, owes)
		for i := range owes {
			for j := range owes {
...
func TestTheOrderDecksAreGivenInDoesNotChangeTheShares(t *testing.T) {
	...
	rapid.Check(t, func(t *rapid.T) {
		...
		was, now := review.GetShares(budget, owes), review.GetShares(budget, otherwise)
		slices.Sort(was)
		slices.Sort(now)
...
func TestDecksLevelOnTheFractionAreSplitByWhatTheyOwe(t *testing.T) {
	...
	if got := review.GetShares(324, owes); !slices.Equal(got, want) {
