// Package gomlx embeds text with a model running on this machine: an ONNX
// sentence encoder on GoMLX's Go backend. There is no library to fetch and
// nothing outside the binary, so a platform ONNX Runtime is not published for
// still makes its own vectors.
package gomlx
...
// backendName is GoMLX's pure-Go backend.
...
//
// identity is what the vectors this model returns are kept under, which the
// settings decide.
...
//
// A batch is laid out at a bucketed length and padded out to a full one: a
// graph is compiled per shape, and the shapes are held to a few.
