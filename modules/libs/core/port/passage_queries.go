...
type PassageQueries interface {
	...
	Lexical(ctx context.Context, vaultID, query string, kinds []domain.SourceKind, limit int, growing bool) ([]domain.Passage, error)
	...
	Named(ctx context.Context, vaultID, query string, kinds []domain.SourceKind, limit int, growing bool) ([]domain.Passage, error)
	...
	Nearest(ctx context.Context, vaultID, recipe string, query []float32, kinds []domain.SourceKind, limit int, floor float64) ([]domain.Passage, error)
}
