...
type VectorQueries interface {
	// Unembedded is at most `limit` small chunks of one vault with no vector
	// from the model given, from `after` onwards, and the cursor to ask from
	// next. Asked again with that cursor, it carries on where it stopped.
	//
	...
	Unembedded(ctx context.Context, vaultID domain.VaultID, model EmbeddingModel, after ChunkCursor, limit int) ([]domain.Passage, ChunkCursor, error)
}
