/**
 * The words the pill saying how much is waiting is drawn with. No DOM, no
 * measurement, no clock.
 */

/** The words the pill is drawn with, declared once. */
export interface OwedWords {
  /** The figure and what it counts, as one reading. */
  readonly counted: (waiting: number) => string
  /** What is said while the figure is still being worked out. */
  readonly counting: string
}

export const OWED_WORDS: OwedWords = {
  counted: (waiting) => `${waiting} to review`,
