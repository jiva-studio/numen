...
const account = (held: Partial<Day> = {}) => mount(Summary, { props: { day: day(held), words: WORDS } })

/** Each of the four that was said, and how many of it. */
const four = (drawn: ReturnType<typeof account>) =>
  drawn.findAll('[data-summary="four"] li').map((one) => [
    one.attributes('data-tone'),
    one.get('[data-summary="said"]').text(),
    one.get('[data-summary="how-many"]').text(),
  ])

describe('a day that has been answered on', () => {
  it('names the day in the window’s own words', () => {
    expect(account({ did: 3 }).get('[data-summary="day"]').text()).toBe('on 2026-03-11')
  })

  it('counts what was answered', () => {
    expect(account({ did: 26 }).get('[data-summary="count"]').text()).toBe('26 answered')
  })
  ...
  it('says nothing of the four where none of them was pressed', () => {
    expect(account({ did: 5 }).find('[data-summary="four"]').exists()).toBe(false)
  })
})

describe('the share that came back', () => {
  it('is what came back of what was asked, to the nearest whole', () => {
    expect(account({ did: 26, asked: 24, recalled: 20 }).get('[data-summary="came"]').text()).toBe(
      '83% came back',
    )
  })
  ...
  it('is not given where nothing being reviewed was asked', () => {
    expect(account({ did: 12, asked: 0, recalled: 0 }).find('[data-summary="came"]').exists()).toBe(false)
  })

  it('is the whole of it where everything asked came back', () => {
    expect(account({ did: 9, asked: 9, recalled: 9 }).get('[data-summary="came"]').text()).toBe(
      '100% came back',
    )
  })
})
...
describe('a day nobody answered on', () => {
  it('says so, and says nothing else', () => {
    const drawn = account()
    expect(drawn.get('[data-summary="count"]').text()).toBe('Nothing answered')
    expect(drawn.find('[data-summary="four"]').exists()).toBe(false)
    expect(drawn.find('[data-summary="came"]').exists()).toBe(false)
  })
})

describe('a day still to come', () => {
  it('says what falls on it rather than what was done on it', () => {
    expect(account({ ahead: true, did: 7 }).get('[data-summary="count"]').text()).toBe('7 to come')
  })
  ...
  it('says nothing falls on a day nothing falls on', () => {
    expect(account({ ahead: true }).get('[data-summary="count"]').text()).toBe('Nothing answered')
  })
})
