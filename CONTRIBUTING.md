...
Two dimensions, and nothing else. A label answers *what this is* or *what part of the repo it touches* — never *when it should be done*.

- **Type**, exactly one: `adr`, `task`, `bug`, `epic`.
- **Area**, zero or more: `desktop`, `mobile`, `domain`, `protocol`, `vault`, `repo`. Every one of them is a commit scope too, which is one vocabulary for commits, issues and pull requests.
- **`needs-decision`** — the only exception: the issue is blocked on a decision, not on work.

Beside these stand the labels the robots put on: `dependencies`, `github_actions` and `javascript`, which are Dependabot's own and are not written by hand.

Priority and status live in the project board as fields, not as labels. A label cannot be sorted, cannot be a column, and turns into archaeology the moment someone forgets to remove it.
...
Decisions live in [`docs/adr/`](docs/adr/README.md) — the index has the reading order and the rules for adding one. Progress is tracked in [issue #13](https://github.com/jiva-studio/numen/issues/13).
